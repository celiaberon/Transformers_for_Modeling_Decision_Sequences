    ...
    for _ in range(val_loader.batches_per_epoch):
        ...
        with torch.no_grad():
            ...
            if predict:
                ...
                if policy == 'argmax':
                    pred_tokens = torch.argmax(last_logits, dim=-1)  # (batch_size)
                elif policy == 'softmax':
...
def update_predictions_file(model_name, starting_step):
    ...
    # Write back only lines up to the cutoff step
    with open(pred_file, 'w') as f:
        for line in lines:
...
def main():    
    ...
    args = parse_args()
    ddp = DDPConfig()
    # Redeclare variables in case they don't get passed through -- check if necessary
    ...
    assert total_batch_size % (B * T * ddp.world_size) == 0, (
        "Total batch size must be divisible by B * T * world_size")
    ...
    if ddp.master_process:
        logger.info(f"Steps to next checkpoint: {next_checkpoint_step}")
    ...
    for step in range(starting_step, max_steps):
        ...
        if (step % next_checkpoint_step == 0) and ddp.master_process:
            ...
            if args.checkpoint_interval == 'log':
                ...
                logger.info(f"Number of steps to next checkpoint: {next_checkpoint_step}")
